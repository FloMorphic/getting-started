...
| --- | --- | --- |
| **Workflows** | `flo_list_workflows`, `flo_get_workflow`, `flo_export_workflow`, `flo_import_workflow` | List and read workflows; export one as the **portable workflow document** (the editor's export file — a readable graph patch under a small header, about half the size of the raw FlowRecord, with no install-local identity) and import one (the same road as the editor's Import dialog: plugin nodes re-stamped by action, unresolved ones listed in `missingActions`; `dryRun` validates and compile-checks without saving). |
| **Designer (AI authoring)** | `flo_get_design_guide`, `flo_plan_patch`, `flo_apply_patch` + the `flo_design_workflow` **prompt** | Author a flow the way the app's *build-with-AI* dialog does: read the designer guide, write a readable **graph patch**, plan it (compile, no save) or apply it (save). Both land through the same import road, so plugin nodes are stamped and missing actions reported here too. |
